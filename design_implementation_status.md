...
1. **Family-shared journal RLS** — `journal_reactions` only allows the child's owner today. When milestones / activities open up to family members, expand both the parent-row and reaction policies in lockstep so Mormor can actually send a heart from her own account.
